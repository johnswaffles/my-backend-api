...
func apply_cycle(day: int, simulation_clock: float, window_bands: Array, town_light_level: float = 0.0, ambient_light_scale: float = 1.0) -> void:
	...
	var sky_top: Color = Color(0.58, 0.78, 0.88).lerp(Color(0.025, 0.035, 0.075), night_amount)
	var sky_horizon: Color = Color(0.94, 0.83, 0.66).lerp(Color(0.05, 0.06, 0.11), night_amount)
	if world_environment and world_environment.environment:
		var env: Environment = world_environment.environment
		env.background_mode = Environment.BG_SKY
		env.background_color = sky_top.lerp(sky_horizon, 0.08)
		env.ambient_light_color = Color(0.73, 0.79, 0.7).lerp(Color(0.16, 0.2, 0.32), night_amount)
		env.ambient_light_energy = lerpf(0.065, 0.13, daylight_curve)
		env.fog_enabled = false
		env.fog_light_energy = 0.0
		env.fog_density = 0.0
		env.glow_enabled = true
		env.glow_bloom = lerpf(0.035, 0.006, daylight_curve)
		env.glow_intensity = lerpf(0.22, 0.035, daylight_curve)
		env.adjustment_enabled = true
		env.adjustment_brightness = lerpf(0.72, 0.84, daylight_scale)
		env.adjustment_contrast = lerpf(1.15, 1.09, daylight_scale)
		env.adjustment_saturation = lerpf(0.92, 0.97, daylight_scale)
	...
	if sun:
		...
		sun.light_energy = lerpf(0.045, 0.42, daylight_curve)
