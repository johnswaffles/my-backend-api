...
func apply_cycle(day: int, simulation_clock: float, window_bands: Array, town_light_level: float = 0.0, ambient_light_scale: float = 1.0) -> void:
	...
	if world_environment and world_environment.environment:
		...
		env.ambient_light_color = Color(0.86, 0.86, 0.76).lerp(Color(0.16, 0.2, 0.32), night_amount)
		env.ambient_light_energy = lerpf(0.12, 0.9, daylight_curve)
		env.fog_enabled = false
		env.fog_light_energy = 0.0
		env.fog_density = 0.0
		env.glow_enabled = true
		env.glow_bloom = lerpf(0.0, 0.018, daylight_curve)
		env.glow_intensity = lerpf(0.0, 0.085, daylight_curve)
		env.adjustment_enabled = true
		env.adjustment_brightness = lerpf(0.76, 1.0, daylight_scale)
		env.adjustment_contrast = lerpf(1.14, 1.1, daylight_scale)
	...
	if sun:
		sun.light_color = Color(1.0, 0.84, 0.58).lerp(Color(0.58, 0.66, 0.86), night_amount)
		sun.light_energy = lerpf(0.08, 1.58, daylight_curve)
	...
	if fill_light:
		...
		fill_light.light_energy = lerpf(0.06, 0.17, daylight_curve)
